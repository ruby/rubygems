  ...
  class CLI::Bump
    ...
    def resolve_targets(targets)
      ...
      targets.each do |name, pin|
        ...
        unless current_spec
          Bundler.ui.warn "Bundler attempted to bump #{name} but it is not in the bundle for the current platform, skipping."
          next
        end

        active_spec = definition.resolve.find_by_name_and_platform(name, current_spec.platform)
        unless active_spec
          Bundler.ui.warn "Bundler attempted to bump #{name} but it could not be resolved, skipping."
          next
        end

        unless active_spec.source.is_a?(Source::Rubygems)
          Bundler.ui.warn "Bundler attempted to bump #{name} but it comes from #{active_spec.source.class}, skipping."
          next
        end
    ...
    def warn_if_unchanged(names, previous)
      names.each do |name|
        ...
        if new_spec.version == previous[name]
          Bundler.ui.warn "Bundler attempted to bump #{name} but its version stayed the same"
